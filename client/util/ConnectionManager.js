...
import { findServerIP, getServerURL } from './FindIP';
...
class ConnectionManager {
  ...
  _startHeartbeat() {
    ...
    this._heartbeatTimer = setInterval(async () => {
      ...
      if (!(await this._ping(this.serverURL))) {
        this._stopHeartbeat();
        this._setStatus('reconnecting');
  ...
  async _attemptReconnect() {
    ...
    if (this.serverURL && (await this._ping(this.serverURL))) {
      this._onConnected();
      return;
    }
    ...
    try {
      ...
      if (ip) {
        this.serverURL = `http://${ip}:${SERVER_PORT}`;
        this._onConnected();
        return;
      }
    } catch { /* ignore */ }
